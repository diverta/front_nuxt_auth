        ...
        <v-col>
          <v-table class="elevation-1">
            <template #default>
              <tbody>
                <tr
                  v-for="[keyName, value] in Object.entries(detail.profile)"
                  :key="keyName"
                >
                  <td width="200">
                    {{ keyName }}
...
const fetchProfile = async () => {
  ...
  try {
    ...
    detail.value = {
      ...
      location: detailsObj.address1
        ? `${detailsObj.address1}, ${detailsObj.address2} ${detailsObj.address3}`
        : "",
      phone: detailsObj.tel,
      email: detailsObj.email,
      url: detailsObj.profileimage.url
      ...
      multiple_check:
        detailsObj?.multiple_check?.map(({ label }) => label)?.join(", ") || "",
      profile: {
